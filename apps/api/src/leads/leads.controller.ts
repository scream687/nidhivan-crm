...
export class LeadsController {
  ...
  kpis(@CurrentUser() user: any) {
    return this.metrics.getDashboardKpis(user);
  }

  @Get("pipeline-health")
  @Roles(Role.ADMIN, Role.MANAGER)
  pipelineHealth() {
    return this.metrics.getPipelineHealth();
  }
  ...
  merge(
    ...
    @Body("duplicateIds") duplicateIds: string[],
    @CurrentUser("id") userId: string,
  ) {
    return this.leads.mergeLeads(primaryId, duplicateIds, userId);
  }
  ...
  getWorkspace(@Param("id") id: string, @CurrentUser() user: any) {
    return this.leads.getWorkspace(id, user);
  }

  @Get(":id/booking")
  getBooking(@Param("id") id: string) {
    return this.bookings.getBookingByLeadId(id);
  }
